...
//declare the TCP connection to the overlay as global variable
int overlay_conn;
...
//
// Pseudocode
// 1) Initialize tcb entries
// 2) Start seghandler thread
//
...
//
// Pseudocode
// 1) Get sockfd for this connection
// 2) Get a tcb entry
// 3) Initialize the tcb entry
// 4) Allocate the mutex for the sendBuf
//
...
//
// Pseudocode
// 1) Get tcb entry that corresponds to this socket
// 2) Switch (clienttcb->state)
//      case CLOSED:
//        1) Set server port in tcb entry
//        2) Fill out a syn header with server info
//        3) Set client tcb state to SYNSENT
//        4) Try to send syn SYN_MAX_RETRY times
//        5) Set tcb state to CLOSED
//      case (SYNSENT,CONNECTED,FINWAIT)
//        return -1
//
...
//
// Pseudocode
// 1) Get tcb entry for this socket
// 2) switch(tcb state)
//      case (CLOSE,SYNSENT,FINWAIT): return -1
//      case CONNECTED:
//        1) Find number of segments needed for given length (len/MAX_SEG_LEN)
//        2) for each segment
//             Allocate a segBuf_t
//             Set segBuf header src and dest port
//             Set segBuf length
//             Set segBuf header type to DATA 
//             Copy data into segBuf
//             Add segBuf to segment buffer in tcb
//
...
//
// Pseudocode
// 1) Get tcb entry for this scoket
// 2) switch(tcb state)
//      case (CLOSED,SYNSENT,FINWAIT): return -1
//      case CONNECTED:
//        1) Fill out fin header with type FIN, and ports
//        2) Set client tcb state to FINWAIT
//        3) Try sending FIN message FIN_MAX_RETRY times
...
// 1) Get tcb entry for this socket
// 2) switch(tcb state)
//      case (SYNSENT,CONNECTED,FINWAIT): return -1
//      case CLOSED:
//        free mutex for the buffer in the tcb
//        free the tcb entry for this connection and set to NULL
...
// Pseudocode
// while(1)
//  receive segment from snp
//  get tcb entry for corresponding to dest port in segbuf header
//  switch(tcb state)
//    case CLOSED: break
//    case SYNSENT:
//      if segBuf.header.type == SYNACK && tcb.serverport == segBuf.header.srcport
...
//    case FINWAIT:
//      if segBuf.type == FINACK && tcb.svrport==segBuf.srcport
...
void* seghandler(void* arg) {
	...
	while(1) {
		//receive a segment
		if(snp_recvseg(overlay_conn,&segBuf)<0) {
			close(overlay_conn);
			pthread_exit(NULL);
		}
		...
		switch(my_clienttcb->state) {
			...
			case SYNSENT:
				...
				break;
			case CONNECTED:	
				if(segBuf.header.type==DATAACK&&my_clienttcb->svr_portNum==segBuf.header.src_port) {
					if(segBuf.header.ack_num >= my_clienttcb->sendBufHead->seg.header.seq_num) {
...
/*******************************************************/
//
// helper functions: send buffer operations 
//
/*******************************************************/
...
void sendBuf_send(client_tcb_t* clienttcb) {
	...
	while(clienttcb->unAck_segNum<GBN_WINDOW && clienttcb->sendBufunSent!=0) {
		snp_sendseg(overlay_conn, (seg_t*)clienttcb->sendBufunSent);
...
void sendBuf_timeout(client_tcb_t* clienttcb) {
	...
	for(i=0;i<clienttcb->unAck_segNum;i++) {
		snp_sendseg(overlay_conn, (seg_t*)bufPtr);
		struct timeval currentTime;
		gettimeofday(&currentTime,NULL);
