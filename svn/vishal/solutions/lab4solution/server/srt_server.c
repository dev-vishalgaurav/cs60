...
//
// Pseudocode
// 1) Initialize tcb entries to NULL
// 2) Set global connection
// 3) Start seghandler thread
//
...
//
// Pseudocode
// 1) Get a new tcb entry for this connection
// 2) Create recieve buffer
// 3) Allocate and initialize mutex lock for receive buffer
// 4) Initialize server tcb to CLOSED state and point mutex and recvBuf
//    to newly allocated data
//
...
// 2) switch(tcb state)
//      case (LISTENING,CONNECTED,CLOSEWAIT): return -1
//      case CLOSED:
//        tcb.state = LISTENING
//        while 1
//          if(tcb.state == CONNECTED) break
//          else busy-wait for a bit
//
...
// 1) Get tcb entry for this socket
// 2) switch(tcb state)
//      case (CLOSED,LISTENING,CLOSEWAIT): return -1
//      case CONNECTED:
//        while 1
//          if(tcb.usedBufLen < length) sleep(RECVBUF_POLLING_INTERVAL)
//          else
//            lock tcb.bufMutex
//            char* dest = buf
//            copy tcb.recvBuf to dest
//            copy copy (tcb.rcvBuf+len) to tcb.rcvBuf, tcb.usedBufLen-len bytes worth
//            tcb.usedBufLen = tcb.usedBufLen - len
//            unlock tcb.bufMutex
//            break
//
...
//        free tcb.recvBuf
//        free tcb entry
//        set tcb entry in tcb table to NULL
//        return 1
//
...
// Pseudocode
// 1) While 1
//      receive segment segBuf
//      get tcb entry from the port in segBuf header dest port
//      switch(tcb state)
//        case CLOSED: break
//        case LISTENING:
//          if(segBuf.type == SYN)
//            tcb.clientport = segbuf.srcport
//            send SYNACK to client
//            tct.state = CONNECTED
//          break
//        case CONNECTED:
//          if(segbuf.type == SYN && tcb.clientport == segBut.srcport
//            duplicate SYN received
//            update tcb about this
//          elseif(segBuf.type == DATA && tcb.clientport == segBuf.srcport)
//            update tcb with data received (data_recieved call)
//          elseif(segBuf.type == FIN && tcb.clientport == segBuf.srcport)
//            tcb.state = CLOSEWAIT
//            start thread for closewait timer
//            send fin received to client (fin_received call)
//        case CLOSEWAIT:
//          if(segBuf.type == FIN && tcb.clientport == segBuf.srcport)
//            fin_received(tcb,&segBuf)  i.e send FIN back
//
	...
	while(1) {
		...
		switch(my_servertcb->state) {
			...
			case CONNECTED:	
				...
				else if(segBuf.header.type==DATA&&my_servertcb->client_portNum==segBuf.header.src_port) {
